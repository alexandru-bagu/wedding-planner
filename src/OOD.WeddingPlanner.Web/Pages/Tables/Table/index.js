$(async function () {
  ...
  var createModal = new abp.ModalManager({
    viewUrl: abp.appPath + 'Tables/Table/CreateModal',
    scriptUrl: "/Pages/Tables/Table/table.js",
    modalClass: "tableModal"
  });
  var editModal = new abp.ModalManager({
    viewUrl: abp.appPath + 'Tables/Table/EditModal',
    scriptUrl: "/Pages/Tables/Table/table.js",
    modalClass: "tableModal"
  });
  ...
  function Guest(table) {
    ...
    guest.invitee.subscribeChanged(async function (current, previous) {
      if (previous) {
        vm.allInvitees()
          .filter(function (p) { return p.id == previous })[0]
          .tableId(null);
      }
      if (current) {
        vm.allInvitees()
