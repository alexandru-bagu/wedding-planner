...
(function ($) {
    ...
    abp.ModalManager = (function () {
        ...
        return function (options) {
            ...
            var _onOpen = function (onOpenCallback) {
				(function() {
					...
					removeCallbacks = function(){
						_onOpenCallbacks.remove(onOpenCallback);
						_onCloseCallbacks.remove(removeCallbacks);
					};
					_onOpenCallbacks.add(onOpenCallback);
					_onCloseCallbacks.add(removeCallbacks);
				})();
            };
			...
			var _off = function(type, callback) {
				if(type == "onOpen") 
					_onOpenCallbacks.remove(callback);
				if(type == "onClose") 
					_onCloseCallbacks.remove(callback);
				if(type == "onResult") 
					_onResultCallbacks.remove(callback);
			}
			var _on = function(type, callback) {
				if(type == "onOpen") 
					_onOpenCallbacks.add(callback);
				if(type == "onClose") 
